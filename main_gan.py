...
from models import infoGenerator2D, infoDiscriminator2D
...
if __name__ == '__main__':
    ...
    # models
    D = infoDiscriminator2D(FG.c_code).to('cuda:{}'.format(FG.devices[0]))
    G = infoGenerator2D(FG.z_dim, FG.c_code).to('cuda:{}'.format(FG.devices[0]))
    ...
    for epoch in range(FG.num_epochs):
        ...
        for step, data in enumerate(trainloader):
            ...
            # x = x.unsqueeze(dim=1).cuda(device, non_blocking=True)
            # printers['inputs0']('input0', (x*0.5+0.5)[0,:,:,:])
            printers['inputs0']('input0', (x*0.5+0.5)[[0],:,:,:,:])

            # D networks
            optimizerD.zero_grad()
            fake0, fake1 = G(z, C, 0), G(z, C, 1)
            printers['outputs0']('train_fake', fake0[0,:,:,:])
            printers['outputs1']('train_fake', fake1[0,:,:,:])
            lossD_0, lossAC_0, D_x_0, D_G_z1_0 = D_loss(x[:,:,42,:,:], fake0, y_real, y_fake, label, d)
            # lossD_0.backward(retain_graph=True)
            # lossAC_0.backward(retain_graph=True)
            lossD_1, lossAC_1, D_x_1, D_G_z1_1 = D_loss(x[:,:,:,48,:], fake1, y_real, y_fake, label, d)
            # lossD_1.backward(retain_graph=True)
            # lossAC_1.backward(retain_graph=True)

            lossD = lossD_0+lossD_1
            lossD.backward(retain_graph=True)
            optimizerD.step()
            lossAC = lossAC_0+lossAC_1
            ...
            lossG_0, lossinfo_0, D_G_z2_0 = G_loss(fake0, c, y_real)
            # lossG_0.backward(retain_graph=True)
            # lossinfo_0.backward()
            lossG_1, lossinfo_1, D_G_z2_1 = G_loss(fake1, c, y_real)
            # lossG_1.backward(retain_graph=True)
            # lossinfo_1.backward()

            lossG = lossG_0+lossG_1
            lossG.backward(retain_graph=True)
            optimizerG.step()
            lossinfo = lossinfo_0+lossinfo_1
            lossinfo.backward()
            optimizerinfo.step()
        ...
        printers['DG_z1']('up', epoch+step/len(trainloader), D_G_z1_0)
        printers['DG_z2']('up', epoch+step/len(trainloader), D_G_z2_0)
        printers['D_x']('up', epoch+step/len(trainloader), D_x_0)
        printers['DG_z1']('front', epoch+step/len(trainloader), D_G_z1_1)
        printers['DG_z2']('front', epoch+step/len(trainloader), D_G_z2_1)
        printers['D_x']('front', epoch+step/len(trainloader), D_x_1)

        # G.eval & check fake image
        if (epoch+1)%10 == 0:
            G.eval()
            fixed_C = torch.cat([fixed_c, fixed_d],1).cuda(device, non_blocking=True)
            fake0, fake1 = G(fixed_z, fixed_C,0), G(fixed_z, fixed_C,1)
            printers['fake0']('vla_fake', (fake0*0.5+0.5)[0,:,:,:])
            printers['fake1']('vla_fake', (fake1*0.5+0.5)[0,:,:,:])
            if ((epoch+1) % 50 == 0):
                save_printer0 = Image3D(vis, 'fake_u_e'+str(epoch+1))
                save_printer0('fake_u_e'+str(epoch+1), (fake0*0.5+0.5)[0,:,:,:])
                save_printer1 = Image3D(vis, 'fake_f_e'+str(epoch+1))
                save_printer1('fake_f_e'+str(epoch+1), (fake1*0.5+0.5)[0,:,:,:])
